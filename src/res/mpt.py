...
def export_individual_particle_analysis():
    writer = pd.ExcelWriter('Individual Particle Analysis.xlsx',
                            engine='xlsxwriter')
...
def export_characterization():
    writer = pd.ExcelWriter(
        'Transport Mode Characterization.xlsx', engine='xlsxwriter')
    ...
    data_sheet.write('D3', 'Subdiffusive')
...
# TODO: Get values from config data
def get_transport_mode_ranges(ref):
    immobile_range = [0, 0.2]
    subdiffusive_range = [0.2, 0.9]
    diffusive_range = [0.9, 1.1]
    active_range = [1.1, ]

    if ref == 0:
        return immobile_range
    elif ref == 1:
        return subdiffusive_range
    elif ref == 2:
        return diffusive_range
...
def get_config(config_name):
    ...
    with open('mpt-config.json') as fh:
        config = json.load(fh)
...
# MSD - Parameters
video_len = get_config('video_len')
total_frames = get_config('total_frames')
# video_len = 19.9  # Get data from user / video file
# total_frames = 577  # Get data from user / microscopy
max_time = video_len
